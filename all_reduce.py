...
def train(rank, world_size, model, train_subsets, num_epochs=100, queue=None):
    # Setting environment variables for distributed training using torch
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, targets in train_loader:
            ...
            # Synchronize gradients across multiple processes using ALL-REDUCE
            for param in model.parameters():
                dist.all_reduce(param.grad.data, op=dist.ReduceOp.SUM)  # Aggregate gradients across all processes
                param.grad.data /= world_size                           # Average gradients by no. of processes
